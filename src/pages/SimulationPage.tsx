      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 sm:p-5 shadow-lg space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {(status as any)?.signal_mode === "EMERGENCY_CORRIDOR" ? (
              <div className="flex items-center gap-1.5 bg-red-950/80 border border-red-500 px-3 py-1.5 rounded-lg text-xs font-bold text-red-200 shadow-md animate-pulse">
                <Siren className="w-3.5 h-3.5 text-red-400 animate-bounce" />
                <span>EMERGENCY GREEN CORRIDOR ACTIVE</span>
              </div>
            ) : (status as any)?.signal_mode === "QUANTUM_OPTIMIZED" ? (
              <div className="flex items-center gap-1.5 bg-emerald-950/70 border border-emerald-500/60 px-3 py-1.5 rounded-lg text-xs font-semibold text-emerald-300 shadow-sm">
                <Sparkles className="w-3.5 h-3.5 text-emerald-400 animate-pulse" />
                <span>QAOA QUANTUM SIGNALS ACTIVE</span>
              </div>
            ) : (
              <div className="flex items-center gap-1.5 bg-slate-950/80 border border-slate-800 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-400">
                <span>MODE: {(status as any)?.signal_mode || "ADAPTIVE"}</span>
              </div>
            )}
              ...
              >
                ...
                <div className="pt-2 border-t border-slate-900 grid grid-cols-2 gap-2 text-[11px] font-mono">
                  <div className="text-slate-400">
                    ...
                    {((status as any)?.signal_mode === "QUANTUM_OPTIMIZED" || (inter as any).control_mode === "QUANTUM_OPTIMIZED") && (
                      <span className="ml-1.5 inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-mono font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/40">
                        <Sparkles className="w-2.5 h-2.5" />
                        QAOA: {inter.green_time}s
                      </span>
                    )}
                  </div>
