...
import { VisionYoloPage } from "./pages/VisionYoloPage";
...
export default function App() {
  ...
  // Live Demo & Judge Orientation Modal States
  const [isFullDemoOpen, setIsFullDemoOpen] = useState<boolean>(false);
  ...
  return (
    <div className="flex h-screen w-screen bg-slate-950 text-slate-100 overflow-hidden font-sans">
      ...
      <Sidebar
        ...
        onRunFullDemo={handleOpenFullDemo}
        onOpenJudgeGuide={handleOpenJudgeGuide}
      />
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-slate-950">
          ...
          {currentPage === "yolo-vision" && (
            <VisionYoloPage
              onNavigateToQuantum={() => setCurrentPage("quantum-optimizer")}
              onNavigateToEmergency={() => setCurrentPage("emergency-corridor")}
            />
          )}
      ...
      {/* 3. Floating Live Demo 11-Stage Orchestrator */}
      <LiveDemoController
      ...
      {/* 4. Judge & Evaluator Guide Modal */}
      <JudgeGuideModal
        isOpen={isJudgeGuideOpen}
        onClose={() => setIsJudgeGuideOpen(false)}
        onRunDemo={() => setIsFullDemoOpen(true)}
      />
    </div>
  );
}
