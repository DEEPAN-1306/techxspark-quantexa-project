...
class QuantumTrafficHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path == "/api/comparison/scenarios":
            from backend.app.comparison.comparison_engine import comparison_engine
            self._send_json({"scenarios": comparison_engine.get_scenarios()})
        elif path == "/api/comparison/latest":
            from backend.app.comparison.comparison_engine import comparison_engine
            self._send_json(comparison_engine.get_latest_result())
        else:
    ...
    def do_POST(self):
        ...
        if path == "/api/comparison/run":
            from backend.app.comparison.comparison_engine import comparison_engine
            self._send_json(comparison_engine.run_comparison(
