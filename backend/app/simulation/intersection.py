...
class TrafficIntersection:
    ...
    def step_signal(self, delta_sec: float, neighbor_data: Optional[List[Dict[str, Any]]] = None):
        ...
        """
        if self.control_mode == "EMERGENCY_CORRIDOR":
    ...
    def step(self, delta_sec: float, neighbor_data: Optional[List[Dict[str, Any]]] = None) -> List[TrafficVehicle]:
        ...
        # 2. Discharge queued vehicles if signal is GREEN on this approach
        is_approach_green = (
            ("North-South" in self.current_signal_phase and self.is_ns_corridor) or
            ("East-West" in self.current_signal_phase and not self.is_ns_corridor)
        )
        if is_approach_green and len(self.queue) > 0:
            # Saturation flow discharge rate:
            # Quantum optimization reduces stop-and-go platooning delay through coordinated green waves:
            coordination_mult = 1.30 if self.control_mode == "QUANTUM_OPTIMIZED" else 1.0
            discharge_rate_per_sec = (self.road_capacity / 60.0) * 0.65 * coordination_mult
    ...
    def preempt_for_emergency(self, corridor_phase: str):
        """Preempts signal to green in emergency corridor direction and locks conflicting phase to red."""
        if self.control_mode != "EMERGENCY_CORRIDOR":
            self.pre_emergency_mode = self.control_mode
            self.pre_emergency_phase = self.current_signal_phase
            self.pre_emergency_green = self.green_time
        self.control_mode = "EMERGENCY_CORRIDOR"
        self.current_signal_phase = corridor_phase
        self.phase_state = "GREEN"
        self.is_ns_corridor = ("East-West" not in corridor_phase)
        self.phase_elapsed_sec = 0.0

    def restore_post_emergency(self):
        """Restores intersection signal controller back to pre-emergency optimization mode."""
        self.control_mode = getattr(self, "pre_emergency_mode", "QUANTUM_OPTIMIZED")
        self.current_signal_phase = getattr(self, "pre_emergency_phase", "North-South GREEN")
        self.green_time = getattr(self, "pre_emergency_green", self.green_time)
        self.phase_state = "GREEN"
        self.phase_elapsed_sec = 0.0
