...
class TrafficSignalQUBO:
    ...
    def __init__(
        ...
        self.penalties = {
            "one_hot": 500.0,          # Penalty for selecting != 1 timing per direction
            "conflict": 1.5,           # Phase conflict / cycle length deviation
            "min_green": 12.0,         # Minimum green time violation
            "max_green": 2.0,          # Maximum green time violation
    ...
    def build_qubo_model(
        ...
        # Total cycle target (default 75s, or custom target_cycle)
        # Penalty = lambda_conflict * (sum_k t_k * x_k - target_cycle)^2
        lambda_conflict = self.penalties["conflict"]
        target_cycle = self.target_cycle if self.target_cycle is not None else 75.0 # Target combined green split
        ...
        # 5. Constraint: MAXIMUM GREEN TIME (Must not exceed 50s)
        lambda_max = self.penalties["max_green"]
        max_green_threshold = 50.0
        for i in range(n):
            t = variables[i]["duration_sec"]
        ...
        penalties_applied["max_green"] = {
            "description": "Penalizes excessive phase holding causing starvations (>50s)",
            "lambda": lambda_max,
...
def build_qubo(
    ...
    formulator = TrafficSignalQUBO(weights=weights, penalty_weights=penalties, timing_options=timing_options, target_cycle=target_cycle)
