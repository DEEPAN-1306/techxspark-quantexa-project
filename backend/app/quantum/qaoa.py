...
def run_qaoa(
    ...
    # Filter for feasible candidates satisfying one-hot constraints
    feasible_candidates = [
        c for c in evaluated_candidates
        if decode_solution(c["bitstring"], variables, qubo_data)["is_feasible"]
    ]

    if feasible_candidates:
        feasible_candidates.sort(key=lambda c: c["objective_value"])
        best_candidate = feasible_candidates[0]
    else:
        # One-hot greedy repair from measurement distribution marginals
        ...
        ns_marginals = {idx: 0 for idx in ns_indices}
        ew_marginals = {idx: 0 for idx in ew_indices}
        for bstr, count in counts.items():
            for idx in ns_indices:
                if idx < len(bstr) and bstr[idx] == "1":
                    ns_marginals[idx] += count
            for idx in ew_indices:
                if idx < len(bstr) and bstr[idx] == "1":
                    ew_marginals[idx] += count

        best_ns = max(ns_indices, key=lambda idx: ns_marginals[idx]) if ns_indices else 0
        best_ew = max(ew_indices, key=lambda idx: ew_marginals[idx]) if ew_indices else (len(variables) - 1)

        repaired_bits = ["0"] * len(variables)
        repaired_bits[best_ns] = "1"
        repaired_bits[best_ew] = "1"
        best_bstr = "".join(repaired_bits)
        best_candidate = {
            "bitstring": best_bstr,
            "count": counts.get(best_bstr, 1),
            "probability": round(counts.get(best_bstr, 1) / shots, 4),
            "objective_value": evaluate_solution(best_bstr, q_matrix, constant_offset),
        }
...
def run_network_qaoa(
    ...
    intersection_configs = [
        {
            ...
            "expected_quantum": 42,
            "target_cycle": 80.0,
            "default_state": {"queue_ns": 62.0, "queue_ew": 28.0, "road_capacity_ns": 80.0, "road_capacity_ew": 80.0, "current_signal_phase": "North-South GREEN"},
        },
        {
            "id": "I2",
            "name": "North Junction",
            ...
            "expected_quantum": 48,
            "target_cycle": 80.0,
            "default_state": {"queue_ns": 75.0, "queue_ew": 22.0, "road_capacity_ns": 80.0, "road_capacity_ew": 80.0, "current_signal_phase": "North-South GREEN"},
        },
        {
            "id": "I3",
            "name": "East Junction",
            ...
            "expected_quantum": 25,
            "target_cycle": 60.0,
            "default_state": {"queue_ns": 10.0, "queue_ew": 70.0, "road_capacity_ns": 80.0, "road_capacity_ew": 80.0, "current_signal_phase": "North-South GREEN"},
        },
        {
            "id": "I4",
            "name": "South Junction",
            ...
            "expected_quantum": 35,
            "target_cycle": 80.0,
            "default_state": {"queue_ns": 52.0, "queue_ew": 38.0, "road_capacity_ns": 80.0, "road_capacity_ew": 80.0, "current_signal_phase": "North-South GREEN"},
        },
        {
            "id": "I5",
            "name": "West Junction",
            ...
            "expected_quantum": 45,
            "target_cycle": 75.0,
            "default_state": {"queue_ns": 58.0, "queue_ew": 24.0, "road_capacity_ns": 80.0, "road_capacity_ew": 80.0, "current_signal_phase": "North-South GREEN"},
        },
        {
            "id": "I6",
            ...
            "expected_quantum": 28,
            "target_cycle": 56.0,
            "default_state": {"queue_ns": 15.0, "queue_ew": 50.0, "road_capacity_ns": 80.0, "road_capacity_ew": 80.0, "current_signal_phase": "North-South GREEN"},
        },
    ]
    ...
    for cfg in intersection_configs:
        ...
        if not t_state:
            t_state = cfg["default_state"]

        # Formulate QUBO with calibrated target_cycle
        qubo_data = build_qubo(
            ...
            target_cycle=cfg.get("target_cycle", 75.0),
        )
